...
int main() {
    ...
    if ((pid = fork()) > 0) {
        close(p_fd[0]); // Fermer le descripteur de fichier de lecture dans le processus parent
        writeFileNames();
        pipeSendList(p_fd);
        wait(NULL);
    } else if (pid == 0) {
        close(p_fd[1]); // Fermer le descripteur de fichier d'écriture dans le processus enfant
